...
static void old_epd_fill_circle_helper(int32_t x0, int32_t y0, int32_t r, int32_t corners, int32_t delta,
...
void old_epd_push_pixels(Rect_t area, int16_t time, int32_t color)
{
    ...
    for (int32_t i = 0; i < EPD_HEIGHT; i++)
    {
        ...
        {
            old_epd_switch_buffer();
            memcpy(old_epd_get_current_buffer(), row, EPD_LINE_BYTES);
            old_epd_switch_buffer();
            memcpy(old_epd_get_current_buffer(), row, EPD_LINE_BYTES);
...
void old_epd_clear_area(Rect_t area)
{
    old_epd_clear_area_cycles(area, 4, 50);
}


void old_epd_clear_area_cycles(Rect_t area, int32_t cycles, int32_t cycle_time)
{
    ...
    for (int32_t c = 0; c < cycles; c++)
    {
        ...
        {
            old_epd_push_pixels(area, dark_time, 0);
        }
        for (int32_t i = 0; i < 4; i++)
        {
            old_epd_push_pixels(area, white_time, 1);
        }
    }
}


Rect_t old_epd_full_screen()
{
...
void old_epd_clear()
{
    old_epd_clear_area(old_epd_full_screen());
}
...
void old_epd_draw_hline(int32_t x, int32_t y, int32_t length, uint8_t color, uint8_t *framebuffer)
{
    for (int32_t i = 0; i < length; i++)
    {
        int32_t xx = x + i;
        old_epd_draw_pixel(xx, y, color, framebuffer);
    }
}


void old_epd_draw_vline(int32_t x, int32_t y, int32_t length, uint8_t color, uint8_t *framebuffer)
{
    for (int32_t i = 0; i < length; i++)
    {
        int32_t yy = y + i;
        old_epd_draw_pixel(x, yy, color, framebuffer);
    }
}


void old_epd_draw_pixel(int32_t x, int32_t y, uint8_t color, uint8_t *framebuffer)
{
    if (x < 0 || x >= EPD_WIDTH)
    {
        return;
...
void old_epd_draw_circle(int32_t x0, int32_t y0, int32_t r, uint8_t color, uint8_t *framebuffer)
{
    int32_t f = 1 - r;
    int32_t ddF_x = 1;
    int32_t ddF_y = -2 * r;
    ...
    old_epd_draw_pixel(x0, y0 + r, color, framebuffer);
    old_epd_draw_pixel(x0, y0 - r, color, framebuffer);
    old_epd_draw_pixel(x0 + r, y0, color, framebuffer);
    old_epd_draw_pixel(x0 - r, y0, color, framebuffer);
    ...
    while (x < y)
    {
        ...
        old_epd_draw_pixel(x0 + x, y0 + y, color, framebuffer);
        old_epd_draw_pixel(x0 - x, y0 + y, color, framebuffer);
        old_epd_draw_pixel(x0 + x, y0 - y, color, framebuffer);
        old_epd_draw_pixel(x0 - x, y0 - y, color, framebuffer);
        old_epd_draw_pixel(x0 + y, y0 + x, color, framebuffer);
        old_epd_draw_pixel(x0 - y, y0 + x, color, framebuffer);
        old_epd_draw_pixel(x0 + y, y0 - x, color, framebuffer);
        old_epd_draw_pixel(x0 - y, y0 - x, color, framebuffer);
    }
...
void old_epd_fill_circle(int32_t x0, int32_t y0, int32_t r, uint8_t color, uint8_t *framebuffer)
{
    old_epd_draw_vline(x0, y0 - r, 2 * r + 1, color, framebuffer);
    old_epd_fill_circle_helper(x0, y0, r, 3, 0, color, framebuffer);
}
                            ...
                            uint8_t color, uint8_t *framebuffer)
{
    ...
    while (x < y)
    {
        ...
        {
            if (corners & 1)
                old_epd_draw_vline(x0 + x, y0 - y, 2 * y + delta, color, framebuffer);
            if (corners & 2)
                old_epd_draw_vline(x0 - x, y0 - y, 2 * y + delta, color, framebuffer);
        }
        if (y != py)
        {
            if (corners & 1)
                old_epd_draw_vline(x0 + py, y0 - px, 2 * px + delta, color, framebuffer);
            if (corners & 2)
                old_epd_draw_vline(x0 - py, y0 - px, 2 * px + delta, color, framebuffer);
            py = y;
        }
...
void old_epd_draw_rect(int32_t x, int32_t y, int32_t w, int32_t h, uint8_t color, uint8_t *framebuffer)
{
    old_epd_draw_hline(x, y, w, color, framebuffer);
    old_epd_draw_hline(x, y + h - 1, w, color, framebuffer);
    old_epd_draw_vline(x, y, h, color, framebuffer);
    old_epd_draw_vline(x + w - 1, y, h, color, framebuffer);
}


void old_epd_fill_rect(int32_t x, int32_t y, int32_t w, int32_t h, uint8_t color, uint8_t *framebuffer)
{
    for (int32_t i = x; i < x + w; i++)
    {
        old_epd_draw_vline(i, y, h, color, framebuffer);
    }
}


void old_epd_write_line(int32_t x0, int32_t y0, int32_t x1, int32_t y1, uint8_t color, uint8_t *framebuffer)
{
    ...
    for (; x0 <= x1; x0++)
    {
        if (steep)
        {
            old_epd_draw_pixel(y0, x0, color, framebuffer);
        }
        else
        {
            old_epd_draw_pixel(x0, y0, color, framebuffer);
        }
        err -= dy;
        if (err < 0)
        {
...
void old_epd_draw_line(int32_t x0, int32_t y0, int32_t x1, int32_t y1, uint8_t color, uint8_t *framebuffer)
{
    ...
    if (x0 == x1)
    {
        if (y0 > y1)
            _swap_int(y0, y1);
        old_epd_draw_vline(x0, y0, y1 - y0 + 1, color, framebuffer);
    }
    else if (y0 == y1)
    {
        if (x0 > x1)
            _swap_int(x0, x1);
        old_epd_draw_hline(x0, y0, x1 - x0 + 1, color, framebuffer);
    }
    else
    {
        old_epd_write_line(x0, y0, x1, y1, color, framebuffer);
    }
}


void old_epd_draw_triangle(int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t x2, int32_t y2,
                       uint8_t color, uint8_t *framebuffer)
{
    old_epd_draw_line(x0, y0, x1, y1, color, framebuffer);
    old_epd_draw_line(x1, y1, x2, y2, color, framebuffer);
    old_epd_draw_line(x2, y2, x0, y0, color, framebuffer);
}


void old_epd_fill_triangle(int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t x2, int32_t y2,
                       uint8_t color, uint8_t *framebuffer)
{
    int32_t a, b, y, last;
    ...
    { // Handle awkward all-on-same-line case as its own thing
        ...
        if (x2 < a)
            ...
            b = x2;
        old_epd_draw_hline(a, y0, b - a + 1, color, framebuffer);
        return;
    }
    ...
    for (y = y0; y <= last; y++)
    {
        ...
        */
        if (a > b)
            _swap_int(a, b);
        old_epd_draw_hline(a, y, b - a + 1, color, framebuffer);
    }
...
void old_epd_copy_to_framebuffer(Rect_t image_area, uint8_t *image_data,
                             uint8_t *framebuffer)
{
...
void IRAM_ATTR old_epd_draw_grayscale_image(Rect_t area, uint8_t *data)
{
    old_epd_draw_image(area, data, BLACK_ON_WHITE);
}


void IRAM_ATTR old_epd_draw_frame_1bit(Rect_t area, uint8_t *ptr,
                                   DrawMode_t mode, int32_t time)
{
    ...
    for (int32_t i = 0; i < EPD_HEIGHT; i++)
    {
        ...
        calc_epd_input_1bpp(lp, old_epd_get_current_buffer(), mode);
        old_epd_output_row(time);
        if (shifted)
        {
            memset(line, 0, sizeof(line));
        }
...
void IRAM_ATTR old_epd_draw_image(Rect_t area, uint8_t *data, DrawMode_t mode)
{
...
static void write_row(uint32_t output_time_dus)
{
    ...
    skipping = 0;
    old_epd_output_row(output_time_dus);
}
...
static void skip_row(uint8_t pipeline_finish_time)
{
    ...
    {
        old_epd_switch_buffer();
        memset(old_epd_get_current_buffer(), 0, EPD_LINE_BYTES);
        old_epd_switch_buffer();
        memset(old_epd_get_current_buffer(), 0, EPD_LINE_BYTES);
        old_epd_output_row(pipeline_finish_time);
    ...
    {
        old_epd_output_row(10);
    }
    else
    {
        // old_epd_output_row(5);
        old_epd_skip();
    }
    skipping++;
}
...
static void IRAM_ATTR reset_lut(uint8_t *lut_mem, DrawMode_t mode)
{
    ...
    case WHITE_ON_WHITE:
        ...
        break;
    default:
        ESP_LOGW("old_epd_driver", "unknown draw mode %d!", mode);
        break;
    }
}
...
static void IRAM_ATTR feed_display(OutputParams *params)
{
    ...
    for (int32_t i = 0; i < EPD_HEIGHT; i++)
    {
        ...
        calc_epd_input_4bpp((uint32_t *)output, old_epd_get_current_buffer(),
