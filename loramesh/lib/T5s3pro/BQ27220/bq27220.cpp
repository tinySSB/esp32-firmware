...
bool BQ27220::parameterCheck(uint16_t address, uint32_t value, size_t size, bool update)
{
    if(!(size == 1 || size == 2 || size == 4)) {
        Serial.printf("(%d) bq27220 parameter size error\r\n", __LINE__);
        return false;
    }
    ...
    do {
        ...
        if(update) {
            if(!i2cWriteBytes(CommandSelectSubclass, buffer, size + 2)) {
                Serial.printf("(%d) bq27220 DM write failed\r\n", __LINE__);
                break;
            }
            ...
            if(!i2cWriteBytes(CommandMACDataSum, buffer, size + 2)) {
                Serial.printf("(%d) bq27220 CRC write failed\r\n", __LINE__);
                break;
            }
            // We must wait, otherwise write will fail
            ...
            if(!i2cWriteBytes(CommandSelectSubclass, buffer, 2)) {
                Serial.printf("(%d) bq27220 DM SelectSubclass for read failed\r\n", __LINE__);
                break;
            }
            ...
            if(!i2cReadBytes(CommandMACData, old_data, size)) {
                Serial.printf("(%d) bq27220 DM read failed\r\n", __LINE__);
                break;
            }
...
bool BQ27220::dateMemoryCheck(const BQ27220DMData *data_memory, bool update)
{
    if(update) {
        ...
        if(!i2cWriteBytes(CommandSelectSubclass, (uint8_t*)&cfg_request, sizeof(cfg_request))) {
            Serial.printf("(%d) bq27220 ENTER_CFG_UPDATE command failed", __LINE__);
            return false;
        }
        ...
        while(--timeout > 0) {
            if(!getOperationStatus(&operation_status)) {
                Serial.printf("(%d) bq27220 failed to get operation status, retries left %lu", __LINE__, timeout);
        ...
        if(timeout == 0) {
            Serial.printf(
                "(%d) bq27220 enter CFGUPDATE mode failed, CFG %u, SEC %u", __LINE__,
    ...
    while (data_memory->type != BQ27220DMTypeEnd)
    {
        if(data_memory->type == BQ27220DMTypeWait) {
            ...
            Serial.printf("(%d) bq27220 invalid DM Type\r\n", __LINE__);
        }
        data_memory++;
    }
    ...
    if(update && result) {
        ...
        while(--timeout > 0) {
            if(!getOperationStatus(&operation_status)) {
                Serial.printf("(%d) bq27220 failed to get operation status, retries left %lu\r\n", __LINE__, timeout);
        ...
        // Check timeout
        if(timeout == 0) {
            Serial.printf("(%d) bq27220 exit CFGUPDATE mode failed\r\n", __LINE__);
            return false;
        }
    }
    return result;
...
bool BQ27220::init(const BQ27220DMData *data_memory)
{
    ...
    do{
        ...
        if(data != BQ27220_ID) {
            Serial.printf("(%d) bq27220 invalid Device Number %04x != 0x0220\r\n", __LINE__, data);
            break;
        }
        ...
        if(!operat.reg.INITCOMP || operat.reg.CFGUPDATE) {
            Serial.printf("(%d) bq27220 incorrect state, reset needed\r\n", __LINE__);
        ...
        if(!getControlStatus(&control_status)) {
            Serial.printf("(%d) bq27220 failed to get control status\r\n", __LINE__);
            break;
        }
        if(control_status.reg.BATT_ID != 0) {
            Serial.printf("(%d) bq27220 incorrect profile, reset needed\r\n", __LINE__);
            reset_and_provisioning_required = true;
        }
        ...
        if(!reset_and_provisioning_required) {
            ...
            if(!dateMemoryCheck(data_memory, false)) {
                Serial.printf("(%d) bq27220 incorrect configuration data, reset needed\r\n", __LINE__);
                reset_and_provisioning_required = true;
            }
        }
        ...
        if(reset_and_provisioning_required) {
            if(!reset()) {
                Serial.printf("(%d) bq27220 failed to reset device\r\n", __LINE__);
            }
            ...
            if(!dateMemoryCheck(data_memory, false)) {
                Serial.printf("(%d) bq27220 data memory update failed\r\n", __LINE__);
                break;
            }
        }

        if(!sealAccess()) {
            Serial.printf("(%d) bq27220 seal failed\r\n", __LINE__);
            break;
        }
        result = true;
    } while(0);
...
bool BQ27220::reset(void)
{
    ...
    do{
        ...
        {
            if(!getOperationStatus(&operat)){
                Serial.printf("bq27220 failed to get operation status, retries left %lu\r\n", timeout);
        ...
        if(timeout == 0) {
            Serial.println("bq27220 INITCOMP timeout after reset");
            break;
        }
        Serial.printf("(%d) bq27220 cycles left: %lu\r\n", __LINE__, timeout);
        result = true;
    } while(0);
    return result;
}
...
bool BQ27220::sealAccess(void) 
{
    ...
    do{
        ...
        if(operat.reg.SEC != Bq27220OperationStatusSecSealed)
        {
            Serial.printf("bq27220 seal failed %u\r\n", operat.reg.SEC);
            break;
        }
        result = true;
    } while(0);
...
bool BQ27220::unsealAccess(void) 
{
    ...
    do{
        ...
        {
            Serial.printf("bq27220 unseal failed %u\r\n", operat.reg.SEC);
            break;
        }
        result = true;
    } while (0);
...
bool BQ27220::fullAccess(void) 
{
    ...
    do{
        ...
        if(timeout == 0) {
            Serial.println("bq27220 failed to get operation status");
            break;
        }
        ...
        if(operat.reg.SEC != Bq27220OperationStatusSecUnsealed){
            Serial.printf("(%d) bq27220 not in unsealed state\r\n", __LINE__);
            break;
        }
        ...
        if(!getOperationStatus(&operat)){
            Serial.println("bq27220 status query failed");
            break;
        }
        if(operat.reg.SEC != Bq27220OperationStatusSecFull){
            Serial.printf("bq27220 full access failed %u\r\n", operat.reg.SEC);
            break;
        }
        result = true;
    } while (0);
